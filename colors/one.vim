...
if has('gui_running') || has('termguicolors') || &t_Co == 88 || &t_Co == 256
  ...
  if &background ==# 'dark'
    ...
    let s:syntax_bg_2     = ['#282c34', '234']
    let s:syntax_gutter = ['#636d83', '60']
    let s:syntax_cursor = ['#2c323c', '236']

    let s:syntax_accent = ['#528bff', '69']

    let s:vertsplit    = ['#181a1f', '233']
    let s:special_grey = ['#3b4048', '59']
    let s:visual_grey  = ['#3e4452', '18']
    let s:pmenu        = ['#333841', '59']
  else
  ...
  let s:syntax_fold_bg = s:mono_5

  " }}}

  " Vim editor color --------------------------------------------------------{{{
  call <sid>X('Normal',       s:syntax_fg,     s:syntax_bg_2,      '')
  ...
  call <sid>X('SpellBad',     '', s:syntax_bg_2, 'undercurl')
  call <sid>X('SpellLocal',   '', s:syntax_bg_2, 'undercurl')
  call <sid>X('SpellCap',     '', s:syntax_bg_2, 'undercurl')
  call <sid>X('SpellRare',    '', s:syntax_bg_2, 'undercurl')
  " }}}
